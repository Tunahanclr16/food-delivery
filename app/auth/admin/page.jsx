...
export default function Admin() {
    const formik = useFormik({
        ...
        onSubmit: async (values,actions) => {
            try {
                const res = await axios.post("http://localhost:3000/api/users/admin", {username,password});
                        console.log("basarılır")
            } catch (error) {
                        console.log(error);
            }
        }
        ...
        <div className="container mx-auto">
            <div className="flex flex-col items-center my-60">
                ...
                <form onSubmit={formik.handleSubmit} className="flex flex-col gap-y-2 items-center my-20 md:w-1/2 w-full mx-auto">
                    <Input type="text" value={formik.values.text} name="username" placeholder="Your username" onChange={formik.handleChange} />
